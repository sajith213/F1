<?php
/**
 * Credit Management - Credit Customers List
 * 
 * This file displays a list of credit customers with search, filter, and pagination
 */

// Set page title and include header
$page_title = "Credit Customers";
$breadcrumbs = '<a href="../../index.php">Dashboard</a> / <span class="text-gray-700">Credit Customers</span>';
...
function safe_number_format($value, $decimals = 0) {
    $value = $value ?? 0; // Convert null to 0
...
if (!isset($conn) || !$conn) {
    ...
    // Make sure summary values are not null
    if ($summary) {
...
if ($summary && $summary['total_credit_limit'] > 0) {
    $utilization = ($summary['total_outstanding'] / $summary['total_credit_limit']) * 100;
}
?>
...
<div class="container mx-auto pb-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <h2 class="text-2xl font-bold text-gray-800">Credit Customers</h2>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-white rounded-lg shadow-md p-4 border-l-4 border-blue-500">
            ...
            <p class="text-xs text-gray-500 mt-2">
                Active: <?= safe_number_format($summary['active_customers']) ?> |
                Inactive: <?= safe_number_format($summary['inactive_customers']) ?> |
                Blocked: <?= safe_number_format($summary['blocked_customers']) ?>
            </p>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-4 border-l-4 border-green-500">
            <div class="flex justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-800"><?= $currency_symbol ?> <?= safe_number_format($summary['total_credit_limit'], 2) ?></p>
                </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-4 border-l-4 border-yellow-500">
            <div class="flex justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-800"><?= safe_number_format($utilization, 1) ?>%</p>
                </div>
    ...
    <div class="bg-white rounded-lg shadow-md p-6 mb-6">
        <form method="GET" action="" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div class="flex items-end md:col-span-3 justify-end space-x-3">
                ...
                <a href="credit_customers.php" class="inline-flex justify-center py-2 px-4 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden">
        <div class="flex justify-between items-center px-6 py-4 border-b">
            <h3 class="text-xl font-semibold text-gray-800">
                ...
                <span class="text-sm font-normal text-gray-500 ml-2">(<?= safe_number_format($total_records) ?> customers)</span>
                <?php endif; ?>
            </h3>
        </div>
        ...
        <?php else: ?>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm text-gray-900">
                                <?= $currency_symbol ?> <?= safe_number_format($customer['credit_limit'], 2) ?>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm">
                                ...
                                <span class="<?= $balanceClass ?>">
                                    <?= $currency_symbol ?> <?= safe_number_format($customer['current_balance'], 2) ?>
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm">
                                <?php
                                ...
                                <span class="<?= $creditClass ?>">
                                    <?= $currency_symbol ?> <?= safe_number_format($availableCredit, 2) ?>
                                </span>
                            </td>
            ...
            <div class="px-6 py-4 border-t border-gray-200 flex items-center justify-between">
                <div class="text-sm text-gray-500">
                    Showing <?= safe_number_format($offset + 1) ?> to <?= safe_number_format(min($offset + $per_page, $total_records)) ?> of <?= safe_number_format($total_records) ?> customers
                </div>
                <div class="flex gap-2">
                    <?php if ($page > 1): ?>
